...
from generate_topo import generateTopoFile, PATHS, DELAY, QUEUE_SIZE, QUEUING_DELAY, BANDWIDTH, LOSS, NETEM, JITTER
...
class ExperienceLauncher(object):
    ...
    def launchXp(self, num, **kwargs):
        if kwargs["protocol"] == "tcp":
            ...
            raise Exception("Unknown protocol: " + kwargs["protocol"])
...
def  experimentTopos(topos, xpName, protocol, tmpfs, xpFnct, scheduler, path_1_ban, path_1_rtt, scenario, **kwargs):
    global TEST_DIR
    testDirectory = xpName + "_" + time.strftime("%Y%m%d_%H%M%S") + "_" + scheduler + "_ban" + path_1_ban + "rtt" + path_1_rtt + "_" + scenario
    ...
    for topo in topos:
        ...
        # renyue: for test
        # topoFilename = topoToFilename(topo) + str(i)
        topoFilename = str(i)
        # renyue: end
